...
if uploaded_file_erp and uploaded_file_d365fo and uploaded_file_field_list:
    ...
    for _, row in filtered_relations.iterrows():
        ...
        net.add_node(parent, title=f"<h4>{parent}</h4><p>{', '.join(field_list_df[field_list_df['TABLE_NAME'] == parent]['COLUMN_NAME'].tolist())}</p>")
        net.add_node(child, title=f"<h4>{child}</h4><p>{', '.join(field_list_df[field_list_df['TABLE_NAME'] == child]['COLUMN_NAME'].tolist())}</p>")
