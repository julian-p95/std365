...
if all([uploaded_file_erp, uploaded_file_d365fo, uploaded_file_field_list]):
    ...
    # Ajout des arêtes (en s'assurant que les nœuds existent)
    ...
    for node in top_tables:
        ...
        if not node_info.empty:
            ...
            # Récupération et formatage des informations des colonnes
            columns_info = field_list[field_list['TABLE_NAME'] == node]
            if not columns_info.empty:
                title_str = "Nom table: " + node + "\nChamps: " + ", ".join(columns_info['COLUMN_NAME'].astype(str) + ' (' + columns_info['DATA_TYPE'].astype(str) + ')')
                net.get_node(node)['title'] = title_str
            else:
                net.get_node(node)['title'] = "Nom table: " + node
